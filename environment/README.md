...
answers, ambiguity margins, in-stock variants) and split into disjoint
`train-pool` / `eval` / `eval-ood` (different catalog) sets with an automated
dedup check.
...
env = vf.load_environment(
    ...
    split="eval",          # "train-pool" | "eval" | "eval-ood"
